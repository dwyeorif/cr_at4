...
p_exchange = 1156.69  # Exchange rate initial value
...
while True:
    try:
        if binance_balance('USDT') > (u_count*binance_usd_price(ticker)+50) and upbit.get_balance('KRW-' + ticker) > u_count:  # Premium trading
            if get_premium((binance_usd_price(ticker)*p_exchange), upbit_price(ticker), ticker) > (p_standard+p_gap):  # Above standard
                ...
                if binance_balance('USDT') < (u_count * binance_usd_price(ticker) + 70):  # Reset
                    p_exchange = get_exchange()
                    p_standard = round(get_premium((binance_usd_price(ticker) * p_exchange), upbit_price(ticker), ticker), 8)
        ...
        p_standard = round((p_standard * 0.999) + (get_premium((binance_usd_price(ticker) * p_exchange), upbit_price(ticker), ticker) * 0.001), 8)
        p_cnt = p_cnt + 1
        time.sleep(7)
